...
- 撤销、重做和可取消的后台分析
- 项目原子自动保存、schema 1兼容迁移和手动打开项目
- 播放、暂停、时间轴和原音频预览
- 原视频裁剪框与铺满播放器的最终成片预览切换
- 剪辑工作台布局、四阶段工作流状态和统一交互动效
- 任意帧添加人工修正框后重新传播
- MP4 导出并保留源视频音频
...
项目自动保存在 `~/.local/state/dance-focus/projects/`。应用重新启动时保持空白，可通过“打开项目”手动恢复。项目文件包含提示框、逐帧跟踪状态、姿态锚点、构图参数、镜头关键帧和镜头路径，但不保存ReID人物特征。
...
4. SAM、姿态与ReID分析完成后会自动切换到成片预览，点击“播放成片”检查动态裁剪。
...
9. 点击“导出跟拍成片”，完成后可立即打开经过解码验证的结果文件。
...
当前测试覆盖裁剪几何、SAM掩码边界、局部区间拼接、项目迁移、撤销重做和分段帧映射。项目还完成了以下端到端验证：
